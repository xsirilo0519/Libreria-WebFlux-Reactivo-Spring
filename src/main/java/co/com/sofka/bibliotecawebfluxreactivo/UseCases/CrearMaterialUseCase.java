...
@Service
@Validated
public class CrearMaterialUseCase implements GuardarMaterial{
    private final MaterialRepositorio repositorio;
    private final MaterialMapper materialMapper;
    @Autowired
    public CrearMaterialUseCase(MaterialMapper mapperUtils, MaterialRepositorio repositorio) {
    ...
    public Mono<MaterialDTO> apply(MaterialDTO materialDTO) {
        return repositorio.save(materialMapper.fromDTO(null).apply(materialDTO)).map(materialMapper.fromCollection());
    }
